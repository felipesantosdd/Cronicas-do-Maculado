...
class AppLogic:
    ...
    def browse_for_save_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            parent=self.app,
            caption="Selecionar Arquivo de Save do Elden Ring",
            dir=start_dir,
            filter="Arquivos de Save do Elden Ring (*.sl2 *.co2);;Save Vanilla (*.sl2);;Save Seamless Coop (*.co2)"
        )
        if file_path:
    ...
    def _load_characters_for_save_file(self, save_file_path):
        ...
        self.app.character_slot_combobox.addItem("Selecione um personagem", userData=None)
        ...
        if err:
            ...
            if "ZSTD" in err or "DCX" in err or "SaveParserError" in err or "BND4" in err or "Buckets" in err:
                QMessageBox.warning(
                    self.app,
                    "Formato de Arquivo de Save Incompatível",
                    "Este arquivo de save parece ser de uma versão antiga ou incompatível do Elden Ring.\n\n"
                    "Causas comuns:\n"
                    "• Saves de versões piratas/crackeadas do jogo\n"
                    "• Formato de save desatualizado\n"
                    "• Pasta do Steam ID errada\n\n"
                    "Soluções:\n"
                    "1. Tente a OUTRA pasta numerada em %APPDATA%\\EldenRing\\\n"
                    "   (Pastas diferentes = versões diferentes do jogo)\n\n"
                    "2. Se tiver várias pastas, use aquela em que outros editores de save funcionam\n\n"
                    "3. Para saves antigos: Use o 'Elden Ring Save Manager' do Nexus Mods para\n"
                    "   converter seu save para o formato atual primeiro\n\n"
                    "Formatos suportados: ER0000.sl2 (Vanilla) ou ER0000.co2 (Seamless Coop)"
                )
            else:
                QMessageBox.warning(self.app, "Erro", f"Falha ao ler personagens:\n{err}")
        ...
        for char in sorted(characters, key=lambda x: x.get('slot_index', 0)):
            ...
            self.app.character_slot_combobox.addItem(f"{char_name} (Nível {level})", userData=char)
    ...
    def handle_stats_update(self, data: dict):
        ...
        if active_char_name and selected_char_name and active_char_name != selected_char_name:
            warning_text = f"Aviso: Jogando como '{active_char_name}', mas '{selected_char_name}' está selecionado no aplicativo."
