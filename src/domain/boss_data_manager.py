...
# Caminho base do projeto para fallback de leitura direta do filesystem
_PROJECT_ROOT = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", ".."))
_DATA_BOSSES_DIR = os.path.join(_PROJECT_ROOT, "data", "Bosses")
...
class BossDataManager:
    ...
    def _load_json_file(self, filename):
        ...
        Prioridade:
        1. Arquivo direto no filesystem (data/Bosses/) — permite dados PT-BR sem recompilar Qt resources.
        2. Qt Resource System (:/data/Bosses/) — utilizado na versão compilada/instalada.
        """
        # 1. Tenta carregamento direto do filesystem (PT-BR ou dados atualizados)
        fs_path = os.path.join(_DATA_BOSSES_DIR, filename)
        if os.path.isfile(fs_path):
            try:
                with open(fs_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                if not isinstance(data, dict):
                    print(f"ERROR: Dados em '{fs_path}' não são um dicionário.")
                    return {}
                print(f"  [filesystem] Carregado: {filename}")
                return data
            except Exception as e:
                print(f"ERROR ao ler '{fs_path}': {e}")

        # 2. Fallback: Qt Resource System (versão compilada)
        filepath = f":/data/Bosses/{filename}"
        qfile = QFile(filepath)
        if not qfile.open(QIODevice.OpenModeFlag.ReadOnly | QIODevice.OpenModeFlag.Text):
            print(f"Erro: Não foi possível abrir '{filepath}' (recurso Qt) nem '{fs_path}' (filesystem).")
            return {}
        ...
        try:
            ...
            if not isinstance(data, dict):
                print(f"ERROR: Dados no recurso '{filename}' não são um dicionário.")
                return {}
            print(f"  [qt-resource] Carregado: {filename}")
            return data
        except Exception as e:
            print(f"ERROR ao carregar recurso '{filename}': {e}")
            return {}
        finally:
            qfile.close()
